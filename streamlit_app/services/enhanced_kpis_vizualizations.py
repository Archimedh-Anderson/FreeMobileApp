...
def compute_business_kpis(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if "is_claim" in df.columns:
        ...
        claims_mask = (
            df["category"]
            .astype(str)
            .str.contains("réclamation|claim|complaint", case=False, na=False)
        )
    ...
    if "priority" in df.columns:
        ...
        critical_mask = priority_normalized.str.contains(
            "critique|critical|urgent|très haute", regex=True, na=False
        )
        high_mask = (
            priority_normalized.str.contains("haute|high|élevée", regex=True, na=False)
            & ~critical_mask
        )
        ...
        critical_mask = urgence_normalized.str.contains(
            "critique|critical|urgent|très haute", regex=True, na=False
        )
        high_mask = (
            urgence_normalized.str.contains("haute|high|élevée", regex=True, na=False)
            & ~critical_mask
        )
        critical = int(critical_mask.sum())
        high = int(high_mask.sum())
    ...
    if "Incident principal" in df.columns:
        ...
        if "incident_confidence" in df.columns:
            kpis["incident_distribution"]["avg_confidence"] = float(
                df["incident_confidence"].mean()
            )
        ...
        if "incident_responsable" in df.columns:
            ...
            kpis["responsable_distribution"] = {
                ...
                "top_responsable": (
                    responsable_dist.index[0] if len(responsable_dist) > 0 else "N/A"
                ),
    ...
    if "réclamations" in df.columns:
        ...
        if "reclamation_confidence" in df.columns:
            kpis["reclamations_enriched"]["avg_confidence"] = float(
                df["reclamation_confidence"].mean()
            )
        ...
        if "sentiment" in df_copy.columns:
            for sentiment in df_copy["sentiment"].unique():
                if pd.notna(sentiment):
                    sentiment_data = (
                        df_copy[df_copy["sentiment"] == sentiment]
                        .groupby("date_only")
                        .size()
...
def render_enhanced_visualizations(df: pd.DataFrame, kpis: Dict[str, Any]):
    ...
    has_theme_data = (
        "Thème principal" in df.columns
        or "topics" in df.columns
        or "category" in df.columns
        or "theme" in df.columns
    )
    ...
    if has_theme_data or has_incident_data:
        ...
        with col2:
            st.markdown(
                "#### <i class='fas fa-exclamation-triangle'></i> Distribution des Incidents",
                unsafe_allow_html=True,
            )
