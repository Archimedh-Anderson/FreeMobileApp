...
def _load_classification_modules():
    ...
    modules_status = {"available": False, "modules": {}, "errors": {}, "warnings": []}
...
def _render_header():
    ...
    with col2:
        ...
        status_icon = (
            ...
            else "<i class='fas fa-times-circle' style='color:#EE5A6F;'></i>"
        )
        ...
        st.markdown(
            f"<div style='text-align: center; margin-top: -20px; font-weight: 600;'>{status_icon} {status_text}</div>",
            unsafe_allow_html=True,
        )
    ...
    with col3:
        ...
        st.markdown(
            f"<div style='text-align: center; margin-top: -20px; font-weight: 600;'>{step_icon} {step_display}</div>",
            unsafe_allow_html=True,
        )
    ...
    with st.sidebar:
        ...
        mode_details = {
            ...
            "balanced": {
                ...
                "precision": "88%",
                "time": "2min",
                "desc": "Recommandé - Production",
            },
            "precise": {
                "models": "BERT + Mistral (100%)",
                "precision": "95%",
                "time": "10min",
                "desc": "Analyses critiques",
            },
        }
...
{detail['desc']}
        ...
        if PROVIDER_MANAGER_AVAILABLE and render_provider_selector:
            ...
            st.session_state.config = {"mode": mode, "provider": selected_provider}
        else:
    ...
    try:
        ...
        if "MistralClassifier" not in modules_dict:
            ...
            if "ollama" in str(error_msg).lower() or "import" in str(error_msg).lower():
                ...
                result["solution"] = (
                    "Vérifiez que le module mistral_classifier.py existe et est valide"
                )
            return result
        ...
        if not check_ollama:
            ...
            result["details"] = (
                "Le module MistralClassifier ne fournit pas la fonction de vérification"
            )
        ...
        try:
            ...
            result["solution"] = (
                "Vérifiez que Ollama est installé: pip install ollama\nPuis démarrez: ollama serve"
            )
...
def _check_gemini_availability() -> dict:
    ...
    try:
        ...
        if "GeminiClassifier" not in modules_dict:
            error_msg = (
                modules.get("modules_status", {})
                .get("errors", {})
                .get("GeminiClassifier", "Erreur inconnue")
            )
            result["message"] = "Module GeminiClassifier non chargé"
            result["details"] = str(error_msg)[:100]
            if (
                "google-generativeai" in str(error_msg).lower()
                or "import" in str(error_msg).lower()
            ):
                result["solution"] = "Installation: pip install google-generativeai"
            else:
                result["solution"] = (
                    "Vérifiez que le module gemini_classifier.py existe et est valide"
                )
            return result
            ...
            try:
                ...
                if is_available:
                    ...
                    if not api_key:
                        ...
                        result["details"] = (
                            "La clé API semble invalide ou le service est inaccessible"
                        )
                        result["solution"] = (
        ...
        if api_key:
            ...
            result["details"] = str(e)[:100]
            result["solution"] = (
                "Ajoutez GEMINI_API_KEY=votre_cle dans le fichier .env\nObtenez votre clé sur: https://makersuite.google.com/app/apikey"
            )
    ...
    try:
        ...
        result["details"] = str(e)[:100]
        result["solution"] = (
            "Vérifiez que le module gemini_classifier.py existe et que google-generativeai est installé"
        )
    except Exception as e:
        logger.warning(f"Erreur vérification Gemini: {e}")
        result["details"] = str(e)[:100]
...
def _render_provider_cards():
    ...
    with col2:
        ...
        gemini_bg_gradient = (
            "linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(5, 150, 105, 0.05) 100%)"
            if gemini_available
            else "linear-gradient(135deg, rgba(245, 158, 11, 0.1) 0%, rgba(217, 119, 6, 0.05) 100%)"
        )
...
def _render_classifiers_tab():
    ...
    available_count = len(
        [
            k
            for k in available_modules.keys()
            if k in classificateurs_info and k != "torch_available"
        ]
    )
        ...
        if role_config:
            ...
            for perm_key, perm_label in key_perms.items():
                ...
                icon = (
                    ...
                    if has_perm
                    else "<i class='fas fa-times' style='color:#95A5A6;'></i>"
                )
                st.markdown(f"{icon} {perm_label}", unsafe_allow_html=True)

            st.caption(
                f"<i class='fas fa-th'></i> {len(role_config.features)} features disponibles",
                unsafe_allow_html=True,
            )
    ...
    with st.expander("Importer via une API / un lien sécurisé", expanded=False):
        ...
        if fetch_btn:
            ...
            if token:
                ...
                if (
                    header_key.lower() == "authorization"
                    and header_value
...
def _section_classification():
    ...
    with col3:
        reduction_pct = (
            (1 - stats.get("total_cleaned", 1) / stats.get("total_original", 1)) * 100
                ...
                if use_optimized:
                    try:
                        ...
                        classifier = UltraOptimizedClassifier(
                            batch_size=50, max_workers=4, use_cache=True
                        )
...
def _classify_fallback(df: pd.DataFrame, text_col: str) -> pd.DataFrame:
    ...
    def classify_row(text):
        ...
        if any(w in t for w in ["merci", "super", "bravo", "excellent"]):
            sentiment = "positif"
        elif any(w in t for w in ["panne", "nul", "mauvais", "honte"]):
            sentiment = "negatif"
        else:
            sentiment = "neutre"
        ...
        urgence = (
            "haute"
            if any(w in t for w in ["panne", "urgent", "critique"])
            else "moyenne" if is_claim == "oui" else "faible"
        )

        topics = (
            "fibre"
            if "fibre" in t
            else "mobile" if "mobile" in t else "facture" if "facture" in t else "autre"
        )
...
def _normalize_kpi_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "incident" not in df.columns:
        ...
        invalid_mask = (
            ~df["incident"].isin(valid_incidents)
            & (df["incident"] != "")
            & (df["incident"] != "nan")
        )
...
def _calculate_metrics(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    urgence_haute_count = count_matches(
        df, "urgence", ["haute", "high", "elevee", "élevée", "critique", "critical"]
    )
...
def _section_results():
    ...
    if st.session_state.show_all_indicators:
        ...
        with col3:
            st.metric("Confiance", f"{report.get('confidence_avg', 0):.0%}")
    ...
    with tab7:
        ...
        if available_cols:
            ...
            st.caption(
                f"<i class='fas fa-info-circle'></i> Affichage de {len(df)} lignes",
                unsafe_allow_html=True,
            )
        else:
...
def _render_quality_summary(df: pd.DataFrame, report: Dict[str, Any]):
    ...
    if incidents_detected == 0 and "urgence" in df.columns and "incident" in df.columns:
        ...
        incident_normalized = df["incident"].astype(str).str.lower().str.strip()
        high_urgency_mask = urgence_normalized.isin(
    ...
    with col_q2:
        st.metric(
            ...
            f"{(sentiments_covered/total*100 if total else 0):.1f}%",
...
def _render_analytics_visualizations(df):
    ...
    st.caption(
        "ℹ️ La répartition détaillée des sentiments est disponible dans l'onglet **Sentiment**."
    )
    ...
    if "sentiment" in df.columns:
        ...
        with stat_col2:
            if "is_claim" in df.columns:
                ...
                st.metric(
                    ...
                    f"{claims_count:,}",
                    f"{(claims_count/total*100 if total else 0):.1f}%",
                )
            else:
        ...
        with stat_col3:
            st.metric(
                "Sentiments négatifs",
                f"{neg_count:,}",
                f"{(neg_count/total*100 if total else 0):.1f}%",
            )
        with stat_col4:
            if "urgence" in df.columns:
                urgence_normalized = df["urgence"].astype(str).str.lower().str.strip()
                ...
                st.metric(
                    "Urgence haute",
                    f"{urgent_count:,}",
                    f"{(urgent_count/total*100 if total else 0):.1f}%",
                )
            else:
                st.metric("Urgence haute", "0", "0%")

        st.caption(
            f"<i class='fas fa-info-circle'></i> Total: {len(df)} tweets analysés",
            unsafe_allow_html=True,
        )
                ...
                go.Pie(
                    ...
                    marker=dict(
                        colors=[
                            "#E74C3C",
                            "#E74C3C",
                        ],  # Même couleur coral pour correspondre au screenshot
...
def _render_urgence_chart(df):
    ...
    if "urgence" in df.columns:
        ...
        st.caption(
            f"<i class='fas fa-info-circle'></i> Distribution sur {len(df)} tweets",
            unsafe_allow_html=True,
        )
    else:
...
def _render_topics_chart(df):
    ...
    if "topics" in df.columns:
        ...
        st.caption(
            f"<i class='fas fa-info-circle'></i> Top 10 thèmes sur {len(df['topics'].unique())} différents",
            unsafe_allow_html=True,
        )
...
def _render_incidents_chart(df):
    ...
    if "incident" in df.columns:
        ...
        fig = px.pie(values=counts.values, names=counts.index, title="")
        fig.update_traces(textinfo="label+percent", textfont_size=12)
        fig.update_layout(height=400, showlegend=True)
        st.plotly_chart(fig, use_container_width=True)

        st.caption(
            f"<i class='fas fa-info-circle'></i> {len(df['incident'].unique())} types identifiés",
            unsafe_allow_html=True,
        )
...
def _display_business_dashboard_mistral(df, report):
    ...
    try:
        ...
        st.caption(
            "Indicateurs clés de performance, statistiques détaillées et pourcentages complets"
        )
        ...
        with st.expander("Statistiques Détaillées et Pourcentages", expanded=False):
            ...
            with col1:
                st.metric("Total Tweets", f"{len(df):,}", delta="100%")
