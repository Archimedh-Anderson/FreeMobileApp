...
try:
    ...
    for i, test in enumerate(test_tweets, 1):
        ...
        print(f"\nResults:")
        print(f"  Sentiment:    {result.sentiment} (confidence: {result.metadata['sent_conf']:.2f})")
        print(f"  Reclamation:  {result.reclamation} (confidence: {result.metadata['reclam_conf']:.2f})")
...
try:
    ...
    print(f"   - Gemini API available: {classifier.available if hasattr(classifier, 'available') else False}")
