...
def test_streamlit_app_launch(page):
    ...
    try:
        ...
        if page.locator("text=FreeMobilaChat").count() > 0 or page.locator("text=Classification").count() > 0:
    ...
    try:
        ...
        if "réclamation" in page_content.lower():
            ...
            if reclamation_matches and int(reclamation_matches[0]) > 0:
                print(f"✅ Complaints detected: {reclamation_matches[0]} (case-insensitive matching working)")
                return True
            else:
